...
def filled_shell_model(beta_0, alpha, v_b, w, n_TS, lamb, theta):
    if beta_0 == 1: # survival probability (S) for He+ PUI is assumed as 1
        ...
        S  = torch_exp( -cxsig(v_b) * (NH_density_r - NH_density(r*(w/v_b)**alpha)) )
    v_b_flag  = torch_where(w<v_b, 1.0, 0.0)
    fs_model = 1./(4.*pi) * (beta_0*r_0**2./u_p) * ( (alpha*S)/(r*v_b**3.) ) * (w/v_b)**(alpha-3.) * n_TS * torch_exp( - (lamb/r) * theta/(theta) * (w/v_b)**(-alpha) )
...
def NH_density(r):
    N  = n_H_TS * ( r * torch_exp(- (lamb/r) * theta/sin(theta)) -  (lamb*theta)/sin(theta) * gammaincc(torch_empty_gamma, (lamb/r) * theta/sin(theta) ) )
    return N
...
def save_to_file():
        ...
        f.write(str(n_Hep)+'\n')
        f.write("# PUI H+ fit results: alpha, v_b [m s-1], bkg_rate [s-1], beta_0 [s-1], L, bkg_counts, S \n")
        f.write(str(alpha)+' '+str(v_b_H)+' '+str(bkg_counts.n/counts_per_s)+' '+str(beta_PUI_H)+' '+str(result_PUI_H.fun)+' '+str(bkg_counts.n)+' '+str(S_calc)+'\n')
...
# trajectory, distance, inflow angles, NH velocity vector
    ...
    r = norm(NH)*r_0
    # inflow angles
    theta = radians(abs(NH_ecli[1]-252.2))
    theta_He = radians(abs(NH_ecli[1]-255.7))
...
# for survival probability
        NH_density_r  = n_H_TS * ( r * exp(- (lamb/r) * theta/sin(theta)) -  (lamb*theta)/sin(theta) * sc.gamma(1e-8)*( 1.-sc.gammainc(1e-8, (lamb/r) * theta/sin(theta) ) ) )
        NHe_density_r  = n_He_TS * ( r * exp(- (lamb_He/r) * theta_He/sin(theta_He)) -  (lamb_He*theta_He)/sin(theta_He) * sc.gamma(1e-8)*( 1.-sc.gammainc(1e-8, (lamb_He/r) * theta_He/sin(theta_He) ) ) )
        torch_empty_gamma = torch.zeros(grid_size_PUI**3)
...
# estimation of survival probability
        r_surv = (r*(u_p/v_b_H)**alpha)
        NH_density_r_calc  = n_H_TS * ( r_surv * exp(- (lamb/r_surv) * theta/sin(theta)) -  (lamb*theta)/sin(theta) * sc.gamma(1e-8)*( 1.-sc.gammainc(1e-8, (lamb/r_surv) * theta/sin(theta) ) ) )
        S_calc = exp( -cxsig(v_b_H) * (NH_density_r - NH_density_r_calc) )
